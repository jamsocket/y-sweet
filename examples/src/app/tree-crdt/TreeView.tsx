...
function Arrow(props: { origin: Point, target: Point, offset?: number, animate: boolean }) {
    let length = Math.sqrt((props.target.x - props.origin.x) ** 2 + (props.target.y - props.origin.y) ** 2)
    ...
    let points = [
        [target.x, target.y],
        [target.x + arrowLength * vector[0] + arrowWidth * perp[0], target.y + arrowLength * vector[1] + arrowWidth * perp[1]],
        [target.x + arrowLength * vector[0] - arrowWidth * perp[0], target.y + arrowLength * vector[1] - arrowWidth * perp[1]],
    ]
...
function Tree(props: { root: YTreeNode }) {
    const { dragOrigin, dragTarget, mouseOver, mouseUp } = useDragHelper((origin: LayoutNode, target: LayoutNode) => {
        origin.treeNode.reparent(target.treeNode)
    })

    const [mousePosition, setMousePosition] = useState<Point | null>(null)

    let nodes = useMemo(() => new NodeBox(props.root).getChildren(), [props.root, props.root.tree.structure])
    const height = 600
    const width = 600
    const xScale = (x: number) => x * width
    const yScale = (y: number) => y * height

    const scalePoint = (point: Point) => ({ x: xScale(point.x), y: yScale(point.y), radius: point.radius })
    ...
    return (
        <svg style={{ width, height }} onMouseUp={mouseUp} onMouseMove={(e) => {
            if (e.buttons === 1) {
            ...
            {nodes.map(node => node.parent && (
                <Arrow key={node.treeNode.id()} origin={scalePoint(node)} target={scalePoint(node.parent)} animate={true} />
            ))}
