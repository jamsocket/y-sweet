...
export class NodeBox {
    ...
    constructor(private treeNode: YTreeNode) {
        ...
        this.width = Math.max(1, this.children.reduce((acc, child) => acc + child.width, 0))
        this.height = this.children.reduce((acc, child) => Math.max(acc, child.height), 0) + 1
    }

    private getChildrenInner(xOffset: number, yOffset: number, xSize: number, ySize: number, parent: LayoutNode | null): LayoutNode[] {
