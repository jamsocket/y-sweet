...
impl S3Store {
	pub fn new(region: String, bucket_name: String, prefix: Option<String>, credentials: Credentials) -> Result<Self, anyhow::Error> {
		let endpoint =
			format!("https://s3.dualstack.{}.amazonaws.com", region).parse().expect("endpoint is a valid Url");
		let path_style = rusty_s3::UrlStyle::VirtualHost;
		let bucket = Bucket::new(
			endpoint, path_style, bucket_name, region
...
impl Store for S3Store {
	async fn get(&self, key: &str) -> Result<Option<Vec<u8>>> {
		let object_get = self.bucket.get_object(
			Some(&self.credentials), &self.prefixed_key(key));
	...
	async fn set(&self, key: &str, value: Vec<u8>) -> Result<()> {
		let action = self.bucket.put_object(Some(&self.credentials), &self.prefixed_key(key));
	...
	async fn remove(&self, key: &str) -> Result<()> {
		let action = self.bucket.delete_object(
			Some(&self.credentials), &self.prefixed_key(key));
		let presigned_url = action.sign(self.presigned_url_duration);
		let client = Client::new();
	...
	async fn exists(&self, key: &str) -> Result<bool> {
		let action = self.bucket.head_object(Some(&self.credentials), &self.prefixed_key(key));
		let presigned_url = action.sign(self.presigned_url_duration);
		let client = Client::new();
